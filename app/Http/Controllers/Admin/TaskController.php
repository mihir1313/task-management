...
class TaskController extends Controller
{
   public function index(Request $request, $id = null)
   {
      if (!$request->isMethod('post')) {
         ...
         return view('admin.tasks.tasks')->with(compact('user', 'project'));
      }
   ...
   public function insert(Request $request)
   {
      ...
      if (isset($hid) && $hid != '') {
         ...
         $updateTask->user_id =  isset($post['user']) ? $post['user'] : '';
         ...
         $userProject = UserProject::where('user_id', $post['user'])
            ->where('project_id', $decryptedId)
            ->first();

         if ($userProject) {
            $userProject->updated_at = Carbon::now();
            $userProject->save();
         } else {
            $newUserProject = new UserProject;
            $newUserProject->user_id = isset($post['user']) ? $post['user'] : '';
            $newUserProject->project_id = isset($decryptedId) ? $decryptedId : '';
            $newUserProject->created_at = Carbon::now();
            $newUserProject->save();
         }
